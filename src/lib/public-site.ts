...
import { getListings, mapListingToVelaProperty, type VelaProperty } from './api';
...
export function replaceTemplateBranding(text: string, agencyName: string) {
  ...
  return text
    ...
    .replace(/Vela Armon Australia's/gi, getPossessiveName(normalizedAgencyName))
    .replace(/\bVela Armon Australia\b/gi, normalizedAgencyName)
    .replace(/Vela Armon's/gi, getPossessiveName(normalizedAgencyName))
    .replace(/\bVela Armon\b/gi, normalizedAgencyName)
...
async function buildFallbackAgentProfile(agentSlug: string, agencySlug?: string | null) {
  ...
  return {
    ...
    activeListings,
    soldListings: [] as VelaProperty[],
    rentedListings: [] as VelaProperty[],
  };
}
...
export async function getAgentProfile(agentSlug: string, agencySlug?: string | null) {
  ...
  return {
    ...
    activeListings: Array.isArray(data.activeListings)
      ? data.activeListings.map((listing: any) => mapListingToVelaProperty(listing, agencySlug))
      : [] as VelaProperty[],
    soldListings: Array.isArray(data.soldListings)
      ? data.soldListings.map((listing: any) => mapListingToVelaProperty(listing, agencySlug))
      : [] as VelaProperty[],
    rentedListings: Array.isArray(data.rentedListings)
      ? data.rentedListings.map((listing: any) => mapListingToVelaProperty(listing, agencySlug))
      : [] as VelaProperty[],
  };
}
